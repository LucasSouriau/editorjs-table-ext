...
export default class Table {
    ...
    getColumn(column) {
        return this.table.querySelector(`.${CSS.row}:nth-child(${column})`);
    }
    ...
    addColumn(columnIndex = -1, setFocus = false) {
        ...
        for (let rowIndex = 1; rowIndex <= this.numberOfRows; rowIndex++) {
            ...
            if (columnIndex > 0 && columnIndex <= numberOfColumns) {
                const previousColumnNumber = columnIndex - 1;
                const previousColumnCell = this.getCell(rowIndex, previousColumnNumber);

                if (previousColumnNumber > 0) {

                    // If cell not shown
                    if (previousColumnCell.classList.contains('d-none') === true) {
                        // Get masterCell
                        const masterCellInfos = previousColumnCell.getAttribute('data-master-cell').split('_', 2);
                        const masterCell      = {row: parseInt(masterCellInfos[0]), column: parseInt(masterCellInfos[1])};

                        // If only masterCell column is before column to add
                        // ___ If equal, master cell must has been managed before with row loop *
                        if (masterCell.column < columnIndex) {
                            const masterCellElem = this.getCell(masterCell.row, masterCell.column);
                            const colSpanValue   = parseInt(masterCellElem.getAttribute('colspan'));

                            if ((masterCell.column + colSpanValue) > columnIndex) {
                                cellElem.classList.add('d-none');
                                cellElem.setAttribute('data-master-cell', `${masterCell.row}_${masterCell.column}`);

                                // If only masterCell row is equal to column to add - Make the increase just once per column added
                                if (rowIndex === masterCell.row) {
                                    masterCellElem.setAttribute('colspan', `${colSpanValue + 1}`);
                                }
                            }
                        }
                    } else if (previousColumnCell.hasAttribute('colspan') === true) {
                        const colSpanValue = parseInt(previousColumnCell.getAttribute('colspan'));
                        if ((previousColumnNumber + colSpanValue) > columnIndex) {
                            cellElem.classList.add('d-none');
                            cellElem.setAttribute('data-master-cell', `${rowIndex}_${previousColumnNumber}`);
                            previousColumnCell.setAttribute('colspan', `${colSpanValue + 1}`);
                        }
                    }
                }

                // Loop over all next columns
                for (let nextColumn = columnIndex; nextColumn < this.numberOfColumns; nextColumn++) {
                    const nextCell = this.getCell(rowIndex, nextColumn);
                    // If next column cell is a masterCell
                    if (nextCell.hasAttribute('rowspan') || nextCell.hasAttribute('colspan')) {
                        // Update all linkedCells columns reference
                        this.table
                            .querySelectorAll(`.${CSS.cell}[data-master-cell="${rowIndex}_${nextColumn}"]`)
                            .forEach((linkedCell) => linkedCell.setAttribute('data-master-cell', `${rowIndex}_${nextColumn + 1}`));
                    }
                }

                if (previousColumnNumber > 0) {
                    $.insertAfter(cellElem, previousColumnCell);
                } else {
                    this.getRow(rowIndex).prepend(cellElem);
                }
            } else {
    ...
    updateToolboxesPosition(row = this.hoveredRow, column = this.hoveredColumn) {
        if (!this.isColumnMenuShowing) {
            const hoveredCellElement = this.getCell(this.hoveredRow, this.hoveredColumn);
            const {fromLeftBorder}   = $.getRelativeCoordsOfTwoElems(this.table, hoveredCellElement);
